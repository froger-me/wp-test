...
export default class FailureReporter implements Reporter {
  onTestEnd(test: TestCase, result: TestResult): void {
    ...
    const lines = [
      ...
      `Profile: ${process.env.WP_TEST_E2E_PROFILE ?? 'unknown'}`,
      `Status: ${result.status}`,
      '',
      'WordPress debug log (last 200 lines):',
      readLogTail(process.env.WP_TEST_E2E_DEBUG_LOG),
    ];
...
function readLogTail(logPath: string | undefined): string {
  ...
  const requestedOffset = Number.parseInt(process.env.WP_TEST_E2E_DEBUG_LOG_OFFSET ?? '0', 10);
