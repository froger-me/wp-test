...
Guided setup, all commands whose names begin with `composer test`, and `composer db:pull` keep detailed command output in `.anyape-wp-test-tools/runtime/logs/`. Saved logs are plain text: terminal colours and cursor controls are removed. Starting any test command removes every older `.log` file from that directory before creating one current `test-...log`, so setup, database-copy, and previous test logs do not accumulate. `composer test` keeps its concise stage output unless `-v` is passed. The PHP-only and browser-only test commands continue to show their detailed output while also saving it. A failed stage always prints the exact log path.
